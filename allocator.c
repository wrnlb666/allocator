...
{
    allocator_t *temp = malloc( sizeof ( allocator_t ) );
    if ( !temp )
    {
        perror( "Allocating memory for allocator" );
        abort();
    }
    *temp = (allocator_t) { 0 }; 
    if ( !is_init )
    {
        allocator_list = malloc( sizeof ( allocator_list_t ) );
        if ( !allocator_list )
        {
            perror( "Allocating memory for allocator_list" );
            abort();
        }
...
void* alloc_malloc( allocator_t* allocator, size_t size )
{
    if ( allocator->capacity && allocator->size >= allocator->capacity )
    {
        ...
        allocator->address = realloc( allocator->address, sizeof ( intptr_t ) * allocator->capacity );
        if ( !allocator->address )
        {
            perror( "Reallocating memory for allocator->address" );
            abort();
        }
    }
    ...
    {
        allocator->capacity ++;
        allocator->address = malloc( sizeof ( intptr_t ) * allocator->capacity );
        if ( !allocator->address )
        {
            perror( "Allocating memory for allocator->address" );
            abort();
        }
    }
...
void* alloc_realloc( allocator_t * allocator, void* ptr, size_t size )
{
    ...
    {
        fprintf( stderr, "Trying to alloc_realloc() a ptr to size 0, \nbut the ptr is not in the allocator\n" );
        EXIT_FAILURE;
    }
...
int alloc_free( allocator_t* allocator, void* ptr )
{
    ...
    fprintf( stderr, "Trying to alloc_free() a ptr, \nbut the ptr is not in the allocator\n" );
    EXIT_FAILURE;
    return -1;
}
...
int alloc_free_allocator( allocator_t* allocator )
{
    ...
    fprintf( stderr, "Trying to alloc_free_allocator() an allocator, \nbut the allocator is not in the allocator list\n" );
    EXIT_FAILURE;
    return -1;
}
